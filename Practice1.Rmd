...
hiring manager should look for when a candidate submits a example code of machine 
...
The homework in the class was very practical and focused on the implementation 
of the theory learned in the class so these reports may not go into as much 
detail in writing out every thought someone doing this analysis may have. 
...
The prac_1 dataset has `r nrow(prac_1)` rows and `r ncol(prac_1)` columns. 
All variables are 100% complete, with no missing values so all counts for all 
variables will be 150. 

The outcome or dependent variable "y" has a mean of `r round_tidy(mean(prac_1$y),1)`, 
the standard deviation of `r round_tidy(std_err_y$numeric.sd, 1)`, and a 
median of  `r round_tidy(median(prac_1$y),1)`. The lowest value of "y" is 
...
the standard deviation of `r round_tidy(std_err_x1$numeric.sd, 1)`, and a 
median of  `r round_tidy(median(prac_1$x1),1)`. The lowest value of "x1" is 
...
the standard deviation of `r round_tidy(std_err_x2$numeric.sd, 1)`, and a 
median of  `r round_tidy(median(prac_1$x2),1)`. The lowest value of "x2" is 
`r round_tidy(min(prac_1$x2),1)` and the highest value is `r round_tidy(max(prac_1$x2), 1)`.

The independent variable "x3" has a mean of `r round_tidy(mean(prac_1$x3),1)`, 
the standard deviation of `r round_tidy(std_err_x3$numeric.sd, 1)`, and a 
median of  `r round_tidy(median(prac_1$x3),1)`. The lowest value of "x3" is 
`r round_tidy(min(prac_1$x3),1)` and the highest value is `r round_tidy(max(prac_1$x3), 1)`.

The independent variable "x4" has a mean of `r round_tidy(mean(prac_1$x4),1)`, 
the standard deviation of `r round_tidy(std_err_x4$numeric.sd, 1)`, and a median 
of `r round_tidy(median(prac_1$x4),1)`. The lowest value of "x4" is 
`r round_tidy(min(prac_1$x4),1)` and the highest value is `r round_tidy(max(prac_1$x4), 1)`.

The independent variable "x9" has a mean of `r round_tidy(mean(prac_1$x9),1)`, 
the standard deviation of `r round_tidy(std_err_x9$numeric.sd, 1)`, and a median 
of `r round_tidy(median(prac_1$x9),1)`. The lowest value of "x9" is 
`r round_tidy(min(prac_1$x9),1)` and the highest value is `r round_tidy(max(prac_1$x9), 1)`.

The independent variable "x10" has a mean of `r round_tidy(mean(prac_1$x10),2)`,
the standard deviation of `r round_tidy(std_err_x10$numeric.sd, 1)`, and a 
median of  `r round_tidy(median(prac_1$x10),1)`. The lowest value of "x10" is
`r round_tidy(min(prac_1$x10),1)` and the highest value is `r round_tidy(max(prac_1$x10), 1)`.
...
much as being $\$ 20,000$ in debt every year or having $\$ 1,000,000$ dollars 
in annual revenue? This is the level of certainty these results are giving us. 
